...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['upload_report'])) {
        ...
        $date_str = date('Y-m-d');
        $filename = 'patient-report-' . $date_str . '-' . $patient_id . '.pdf';

        // Directory to store the reports
        $upload_dir = '../Diabetic/';
        $file_path = $upload_dir . $filename;

        // Move the uploaded file to the Diabetic folder
        if (move_uploaded_file($file['tmp_name'], $file_path)) {
            // Insert the report details into the patient-diabetic table
            $diabetic_level = $_POST['diabetic_level']; // Assuming the diabetic level is passed in the form
            $stmt = $db->prepare("INSERT INTO `patient-diabetic` (`Patient ID`, `Diabetic Level`, `Date`, `Report`) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssss", $patient_id, $diabetic_level, $date_str, $filename);
            if ($stmt->execute()) {
                // Update the diabetic level in the patient table
                $update_stmt = $db->prepare("UPDATE `patient` SET `Diabetic_level(Mg/DL)` = ? WHERE `Patient_Id` = ?");
                $update_stmt->bind_param("si", $diabetic_level, $patient_id);
                if ($update_stmt->execute()) {
                    ...
                    echo "<p class='error'>Error updating diabetic level in patient table: " . $update_stmt->error . "</p>";
                }
            } else {
                    ...
                    <?php while ($row = $patients->fetch_assoc()): ?>
                        <li class="patient-item">
                            ...
                            <form method="POST" enctype="multipart/form-data" class="upload-form">
                                ...
                                <input type="text" name="diabetic_level" placeholder="Enter Diabetic Level (Mg/DL)" required>
                                <input type="file" name="report_file" required>
                                <button type="submit" name="upload_report" class="upload-button">Upload Report</button>
                            </form>
                        </li>
                    <?php endwhile; ?>
                </ul>
