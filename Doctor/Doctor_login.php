...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
    ...
    if ($Uname && $Pass) {
        $sql1 = "SELECT * FROM doctors WHERE DoctorID=? OR Email=?";
        ...
        if ($stmt = $con->prepare($sql1)) {
            ...
            if ($result1->num_rows > 0) {
                ...
                $count = $DBUserName["LoginCount"]; // Corrected variable name
                
                // Verify the password using password_verify (for security)
                if ($Pass== $hashedPassword) {  // Use password_verify for hashed passwords
                     
                        header("Location:hospital_select.php?DoctorID=" . urlencode($Uname)); 
...
<head>
    ...
    <title>HealtLanka: Doctor Login</title>
    <style>
